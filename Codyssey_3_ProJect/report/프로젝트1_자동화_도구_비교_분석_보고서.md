...
| 구분 | 경로 | 상태 |
|------|------|------|
| Make 원본 | 프로젝트 루트 `.gif/` | ✅ 6개 확보 |
| Make 보고서용 GIF | `report/gifs/` (`make_*.gif`) | ✅ 영문 별칭 |
| n8n 설치·OAuth 마찰 | `report/png/` (PNG·GIF·README·렌더 스크립트) | ✅ 로그 기반 생성 |
| n8n 실행 촬영 | `report/gifs/n8n_*.gif` | ⬜ **앞으로 촬영** |
...
설치·OAuth 마찰 미디어는 **`report/png/`** 에 분리 보관 (확장자·생성물 구분).
...
#### Make 동작 GIF (촬영 완료 · `.gif/` → `report/gifs/`)
...
> 원본 파일명: `.gif/고액_지출_*`, `일반_지출_*`, `기타_지출_*` (프로젝트 루트).
...
파일을 아래 이름 그대로 `report/gifs/` 에 저장하면 본문에 표시됩니다.
...
| # | 파일 | 상태 |
|---|------|------|
| M1–M6 | Make 고액/일반/기타 × (폼+액션) | ✅ `.gif/` 및 `report/gifs/` |
...
*Make 미디어 반영 완료. n8n GIF만 `report/gifs/` 에 넣으면 제출본에 가깝다.*
